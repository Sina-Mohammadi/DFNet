...
*****Note that this is basically the ImageDataGenerator of Keras, but it
is modified in a way that can generate masks with one-hot encoding*****
...
class DirectoryIterator(Iterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        if self.n_categorical:
            batch_x = np.zeros(
            (len(index_array),self.image_shape[0],self.image_shape[1],self.n_categorical),
            dtype=self.dtype)
